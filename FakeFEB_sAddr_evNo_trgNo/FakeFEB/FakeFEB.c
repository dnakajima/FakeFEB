...
int main(int argc, char *argv[])
{
  ...
  while((opt=getopt_long(argc,argv,"hi:p:t:r:v",options,&index))!=-1){
    ...
    case 'h':
      ...
      printf("%s -i 192.168.10.100 -p 24 -t 22222 \n",argv[0]);
      printf("***** LIST OF OPTIONS *****\n");
      printf("-h|--help 			   : shows usage\n");
      printf("-i|--ipaddr <IP address>		   : ip address to connect and send eventdata to  Camera Server.\n");
      printf("-p|--port   <Port number>		   : port to connect and send eventdata to  Camera Server.\n");
      printf("-t|--trigport <Port number>	   : port to accept trigger packet (broadcast packet)\n");
      printf("-r|--readdepth <ReadDepth>	   : read depth of the event.Default is 30.\n");
    ...
    case 'i':
      ...
      break;
    case 'p':
      dataServPort=(unsigned short)strtoul(optarg,NULL,0);
      p_isset=true;
      break;
    case 't':
      trigRecvPort = optarg;
